  ...
  forEachChild(node, function cb(currentNode: Node) {
    if (isAnOperand(currentNode)) {
      ...
      output.operands._unique.add(
        ((currentNode as Identifier).text ||
          (currentNode as Identifier).escapedText) as string
      );
      ...
      output.operators._unique.add(
        ((currentNode as any).text || currentNode.kind) as never
      );
    }
...
export const getHalsteadForSource = (ctx) => {
  ...
  return getFormattedObj(
    mergeObjectPropertiesBasedOnKeys(halsteadPerFunc),
